...
def get_next_logits(model: Small_LLM_Model,
...
def score_candidate_name(model, prompt: str, candidate_name: str) -> float:
    """
    Score a candidate function name by encoding the prefix up to the function
    name and asking the model for the next-token logits.
    This is not full token-by-token JSON generation, but it does use the
    model's logits and is consistent with constrained prefix scoring.
    Minimal constrained selector:
    - encode the prompt + candidate function name
    - ask the model for the next-token logits
    - score the candidate by the logit of the final token of the candidate name
    """
    text = prompt + " " + candidate_name
    ids = encode_text(model, text)
    if len(ids) < 2:
        return -1e9

    prefix = ids[:-1]
    last_id = ids[-1]
    logits = get_next_logits(model, prefix)
    if last_id >= len(logits):
        return -1e9
    return float(logits[last_id])


def constrained_select_function(prompt: str, registry: List[Dict[str, Any]],
                                model: Small_LLM_Model) -> Optional[str]:
    """
    Deterministic constrained choice over the allowed function names.
    Only function names from the registry are considered.
    Restrict the selection to valid function names only.
    This is still a lightweight logits-based constraint, not a full JSON-state
    decoder.
    """
    allowed = [f["name"] for f in registry if isinstance(f,
                                                         dict) and "name" in f]
    if not allowed:
        return None
    ...
    for name in allowed:
        try:
            score = score_candidate_name(model, prompt, name)
        except Exception:
            continue
        if score > best_score:
            best_score = score
